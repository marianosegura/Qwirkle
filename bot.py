...
class Bot:
    ...
    def get_playable_positions(self, board, turn_moves):
        """ Returns all playable positions of the board, given the player
            ...
            Args:
                ...
                turn_moves(:obj:`TurnMoves`): Tile movements made by a player.
        ...
        if len(turn_moves) >= 1:
            first_tile_position = self.get_tile_position(board, turn_moves.tiles[0])
            if not first_tile_position:
                print("!")
            if len(turn_moves) == 1:
                restriction = TileRestriction('same row or col', first_tile_position)
            if len(turn_moves) >= 2:
                if turn_moves.positions[0].row == turn_moves.positions[1].row:
